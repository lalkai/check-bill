...
import { Tick01Icon, CrownIcon, Cancel01Icon, Calendar03Icon } from "@hugeicons/core-free-icons";
...
const emit = defineEmits(["close", "toggle-item", "toggle-payment-status", "settle-all", "cancel-all"]);
useScrollLock(computed(() => props.show));

const groupColor = computed(() => groupsStore.activeGroup?.color || "#0066cc");

const avatarStyle = computed(() =>
  props.payer?.isOwner ? { backgroundColor: "#f59e0b" } : { backgroundColor: groupColor.value }
);
...
const categoryIconClass = (item) => {
  if (item.paid && !props.payer?.isOwner) return "bg-green-50 text-green-500 border-green-100";
  if (!item.paid && !props.payer?.isOwner) return "bg-orange-50 text-orange-400 border-orange-100";
        ...
        <div class="relative bg-white rounded-[2.2rem] sm:rounded-[2.5rem] shadow-[0_24px_60px_rgba(0,0,0,0.18)] w-full max-w-md border border-white/20 max-h-[85vh] sm:max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div class="px-6 pt-6 pb-5 flex items-center gap-4 border-b border-neutral-100">
            ...
            <div
              ...
              :style="avatarStyle"
            >
              <HugeiconsIcon v-if="payer.isOwner" :icon="CrownIcon" size="22" :stroke-width="2.5" />
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                <h2 class="text-lg font-black text-neutral-900 tracking-tight leading-tight">{{ payer.name }}</h2>
                <span
                  v-if="payer.isOwner"
          ...
          <!-- ── SCROLLABLE BODY ── -->
          <div data-scroll-inner class="flex-1 overflow-y-auto scrollbar-hide px-6 py-5 space-y-4">

            <!-- Total amount card -->
            <div class="rounded-2xl border p-4 flex items-center justify-between" :class="totalBgClass">
              <div>
                <p class="text-[10px] font-bold text-neutral-400 uppercase tracking-widest mb-1">
                  {{ payer.isOwner ? $t("summary.sponsoredTotal") : $t("summary.total") }}
                </p>
                <p class="text-[28px] font-black leading-none" :class="totalAmountClass">
                  <span class="text-base opacity-50 mr-0.5">฿</span>{{ formatCurrency(payer.totalAmountDue) }}
                </p>
              </div>
              <!-- Status pill -->
              <div v-if="!payer.isOwner">
                <span v-if="payer.paid" class="flex items-center gap-1.5 text-[11px] font-black text-green-600 bg-green-100 px-3 py-1.5 rounded-xl">
                  <HugeiconsIcon :icon="Tick01Icon" size="13" :stroke-width="3" />
                  {{ $t("summary.paid") }}
                </span>
            ...
            <div v-if="!payer.isOwner && allDatesArray.length > 0">
              <div class="flex items-center gap-2 mb-3">
                <HugeiconsIcon :icon="Calendar03Icon" size="13" :stroke-width="2.5" class="text-neutral-400" />
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <div class="flex items-center gap-3 px-4 py-3">
                    <div class="flex-1 min-w-0">
                      <p class="text-[10px] text-neutral-400 font-semibold uppercase tracking-widest">{{ dateItem.date }}</p>
                      <p class="text-base font-black text-neutral-800 leading-tight">
                        ฿{{ dateItem.amount ? formatCurrency(dateItem.amount) : "0.00" }}
                      </p>
                    </div>
                    <button
                    ...
                    >
                      <HugeiconsIcon
                        v-if="groupsStore.getPaidStatusByDate(payer.name, dateItem.date)"
                        :icon="Tick01Icon" size="13" :stroke-width="3"
                      />
                      {{ groupsStore.getPaidStatusByDate(payer.name, dateItem.date) ? $t("summary.paid") : $t("summary.markPaid") }}
                    </button>
                  </div>

                  <!-- Bill items for this date -->
                  <div
                    v-for="(item, index) in payer.billItems.filter(b => b.date === dateItem.date)"
                    :key="index"
                  ...
                  >
                    ...
                    <div class="flex-shrink-0">
                      ...
                      >
                        <HugeiconsIcon :icon="Tick01Icon" size="11" :stroke-width="3" />
                      </div>
                    </div>

                    <!-- Icon -->
                    <div class="w-8 h-8 flex-shrink-0 rounded-lg flex items-center justify-center border" :class="categoryIconClass(item)">
                      <HugeiconsIcon :icon="getIcon(item.icon)" size="14" :stroke-width="2.5" />
                    </div>

                    <!-- Text -->
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-black text-neutral-800 truncate" :class="item.paid ? 'line-through text-neutral-400' : ''">
                        {{ item.description }}
                      </p>
                    </div>

                    <!-- Amount -->
                    <span class="text-sm font-black flex-shrink-0" :class="amountClass(item)">
            ...
            <div v-if="payer.isOwner && payer.billItems && payer.billItems.length > 0" class="space-y-2">
              ...
              >
                <div class="flex items-center gap-3">
                  <div class="w-9 h-9 flex-shrink-0 rounded-xl flex items-center justify-center border" :class="categoryIconClass(item)">
                    <HugeiconsIcon :icon="getIcon(item.icon)" size="16" :stroke-width="2.5" />
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-black text-neutral-800 truncate">{{ item.description }}</p>
                    <p class="text-[10px] text-neutral-400 font-semibold mt-0.5">{{ item.date }}</p>
                  </div>
                  <span class="text-sm font-black flex-shrink-0 text-neutral-800">฿{{ formatCurrency(item.amount) }}</span>
                </div>

                <!-- Owner: payers breakdown -->
                <div v-if="item.payers && item.payers.length > 0" class="mt-3 pt-3 border-t border-dashed border-neutral-100">
                  <p class="text-[9px] font-black text-neutral-400 uppercase tracking-widest mb-1.5">{{ $t('summary.payers') }}</p>
                  <div class="flex flex-wrap gap-1.5">
                    ...
                    >
                      <HugeiconsIcon v-if="p.name === groupsStore.activeGroup?.ownerName" :icon="CrownIcon" size="10" :stroke-width="2.5" class="text-amber-500" />
                      <span v-else class="w-1.5 h-1.5 rounded-full" :class="p.paid ? 'bg-green-500' : 'bg-orange-400'"></span>
                      {{ p.name }}
                    </span>
                  </div>
                </div>
          ...
          <!-- ── STICKY FOOTER ── -->
          <div v-if="!payer.isOwner" class="px-6 py-4 border-t border-neutral-100 bg-white flex-shrink-0">
            <button
              v-if="!payer.paid"
