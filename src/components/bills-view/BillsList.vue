...
<template>
  <div class="space-y-4">
    <div v-if="billStore.bills.length === 0"
      ...
      <div
        ...
        <HugeiconsIcon :icon="Invoice01Icon" size="40" class="text-neutral-300" />
      </div>
    ...
    <div v-else class="space-y-4">
      <div class="flex items-center justify-between mb-4 px-2">
        <h2 class="text-[11px] font-black text-neutral-400 uppercase tracking-[0.15em]">
          {{ $t("bills.allExpenses") }}
        </h2>
      </div>
      <BillCard v-for="bill in sortedBills" :key="bill.id" :bill="bill" @remove="removeBill" @edit="openEditModal" />
    </div>
  </div>
</template>
