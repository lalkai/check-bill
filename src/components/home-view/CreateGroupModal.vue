...
<template>
    <Teleport to="body">
        <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <!-- Modal -->
            <div class="relative bg-white rounded-lg shadow-lg w-full max-w-md p-4 animate-modalIn">
                <h2 class="text-xl font-bold text-neutral-700 mb-6">สร้างกลุ่มบิลใหม่</h2>

                <!-- Name Input -->
                <div class="mb-6">
                    <label class="block text-sm font-medium text-neutral-500 mb-2">ชื่อกลุ่ม</label>
                    <input v-model="name" type="text" placeholder="เช่น ทริปเชียงใหม่, ค่าห้อง..." class="a-input"
                        @keyup.enter="handleCreate" ref="nameInput" />
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <button @click="handleCreate" class="a-button-primary flex-1" :disabled="!name.trim()"
                        :class="!name.trim() ? 'opacity-50 cursor-not-allowed' : ''">สร้าง</button>
                </div>
            </div>
        </div>
    </Teleport>
