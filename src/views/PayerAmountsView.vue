...
const sharePayer = () => {
  ...
  filteredPayerAmounts.value.forEach(payer => {
    selectedPayers.value.push(payer.name);
  });
...
const toggleSelectAllPayers = () => {
  if (selectAllPayers.value) {
    selectedPayers.value = filteredPayerAmounts.value.map(payer => payer.name);
  } else {
    selectedPayers.value = [];
  }
};
...
const togglePayerSelection = (payerName) => {
  ...
  selectAllPayers.value = selectedPayers.value.length === filteredPayerAmounts.value.length;
...
const generateShareUrl = async () => {
  try {
    const allPayersData = filteredPayerAmounts.value
      .filter(payer => selectedPayers.value.includes(payer.name))
      .map(payer => {
        const billItems = [];
...
<template>
  <div>
    ...
    <!-- Payer Amounts List -->
    <div class="space-y-4">
      <div v-for="(payer, index) in filteredPayerAmounts" :key="index" class="a-card transition-all duration-300">
      ...
      <div v-if="filteredPayerAmounts.length === 0" class="a-card py-12">
        <div class="text-center text-neutral-400">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M9 12h6m-3-3v6m-6 4h12a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
          </svg>
      ...
      <div class="bg-white rounded-2xl shadow-a-hover w-full max-w-md p-6 m-4">
        ...
        <div v-if="!shareUrl" class="mb-5">
          ...
          <div class="max-h-60 overflow-y-auto border border-neutral-200 rounded-lg p-2">
            <div v-for="payer in filteredPayerAmounts" :key="payer.name"
              ...
              <input type="checkbox" :id="'payer-' + payer.name" :checked="selectedPayers.includes(payer.name)"
                @click="togglePayerSelection(payer.name)" class="h-4 w-4 text-primary border-gray-300 rounded" /> <label
                :for="'payer-' + payer.name" class="ml-2 flex-1 flex justify-between cursor-pointer">
          ...
          <div class="mt-4 flex">
            <button @click="generateShareUrl" class="a-button-primary flex-1" :disabled="selectedPayers.length === 0">
              แชร์ข้อมูลที่เลือก
            </button>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-3">
          <button v-if="shareUrl" @click="openShareLink" class="a-button-primary w-full">
            เปิดลิงก์แชร์
          </button>

          <div v-if="shareUrl" class="flex gap-3">
            <button @click="shareUrl = ''; selectedPayers = []; selectAllPayers = (filteredPayerAmounts.length === 0);"
              class="a-button-secondary flex-1 sm:text-sm">
              ย้อนกลับ
            </button>
            <button @click="showSharePopup = false" class="a-button-secondary flex-1 sm:text-sm">
              ปิด
            </button>
          </div>

          <button v-if="!shareUrl" @click="showSharePopup = false" class="a-button-secondary w-full">
            ปิด
          </button>
        </div>
      </div>
