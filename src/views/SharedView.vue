...
<template>
    ...
    <div v-else-if="allSharedData && allSharedData.payers && allSharedData.payers.length > 0" class="space-y-6">
      <div v-for="payer in allSharedData.payers" :key="payer.name" class="a-card">
        ...
        <div v-if="payer.billItems && payer.billItems.length > 0" class="mt-4 pt-4 border-t border-neutral-200">
          ...
          <div v-if="getHiddenItemsCount(payer) > 0" class="mt-3 text-center">
            <button @click="toggleBillItems(payer.name)"
              ...
              <div class="flex items-center justify-center">
                <svg v-if="!shouldShowAllItems(payer.name)" xmlns="http://www.w3.org/2000/svg" fill="none"
                ...
                {{ shouldShowAllItems(payer.name) ? 'ซ่อนรายการ' : `แสดงเพิ่มเติม (${getHiddenItemsCount(payer)}
                รายการ)` }}
              </div>
            </button>
          </div>
        ...
        <div v-if="overallPromptpayID" class="mt-5 flex justify-center">
          <button @click="payer.paid ? null : openPaymentModal(payer)" class="a-button-primary"
  ...
  <div v-if="showQrCodeModalForPayer"
    ...
    <div
      ...
      <p class="text-center text-neutral-600 mb-1">สำหรับ: <span class="font-medium">{{ showQrCodeModalForPayer.name
      }}</span></p>
      ...
      <div :id="`payment-qrcode-container-${showQrCodeModalForPayer.name.replace(/\s+/g, '-')}`"
