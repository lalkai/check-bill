...
<template>
  ...
  <div class="p-4 bg-base-200 rounded-lg shadow-md">
    <h2 class="text-2xl font-bold mb-6">จัดการบิล</h2>
    
    <!-- เพิ่มบิลใหม่ -->
    <div class="mb-8 bg-base-100 p-4 rounded-lg shadow">
      <h3 class="text-lg font-semibold mb-4">เพิ่มบิลใหม่</h3>
    ...
    <!-- รายการบิล -->
    <div class="space-y-4">
      <div v-for="bill in billStore.bills" :key="bill.id" class="bg-base-100 rounded-lg shadow-sm overflow-hidden">
        <div class="p-4">
        ...
        <div class="bg-base-200 p-4">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="payer in bill.payers" :key="payer.name" class="badge badge-outline">
              {{ payer.name }}
            </span>
          </div>
        </div>
  ...
  <div v-if="editingBillId !== null" class="fixed inset-0 z-50 flex items-center justify-center overflow-x-auto overflow-y-auto bg-black bg-opacity-50">
    <div class="bg-base-100 rounded-lg p-6 max-w-md w-full mx-4">
      ...
      <div class="space-y-4">
        <div class="form-control">
          ...
          <input v-model="editedBillDescription" type="text" class="input input-bordered w-full" />
        </div>
        <div class="form-control">
          <label class="label">
            <span class="label-text">จำนวนเงิน</span>
          </label>
          <input v-model="editedBillAmount" type="number" class="input input-bordered w-full" />
        </div>
        <div class="form-control">
          <label class="label">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
            <span
              ...
              :class="selectedPeople.includes(person.name) ? 'bg-primary text-primary-content' : 'bg-base-200 hover:bg-base-300'"
      ...
      <div class="flex justify-end mt-6 space-x-2">
        ...
        <button @click="saveEditedBill" class="btn btn-primary">
          บันทึก
        </button>
      </div>
    </div>
